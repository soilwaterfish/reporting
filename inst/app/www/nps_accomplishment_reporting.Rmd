...
poly_unique <- readxl::read_xlsx(r'{Z:\GIT\reporting\data\gpas_unique_values.xlsx}',
                                 sheet = 'gpas_polygon_final_list')

line_unique <- readxl::read_xlsx(r'{Z:\GIT\reporting\data\gpas_unique_values.xlsx}',
...
g1 <- report_df_filt_poly %>% ggplot() + geom_col(aes(SPATIAL_SIZE,ACTIVITY_CODE,  fill = ADMIN_ORG_NAME), show.legend = T) +
  resourceviz::custom_theme(font_family = 'special', font_size = 9.5) + 
  colorblindr::scale_fill_OkabeIto() +
  labs(y = '', x = 'Area (acres)', fill = 'National Forest')+
  theme(legend.position = 'right')

ggsave('g1.png',g1, device = 'png', path = 'inst/app/www/images/', width = 6.09, height = 5.11, units = 'in', dpi = 1600)
...
ggsave('g2.png', g2, device = 'png', path = 'inst/app/www/images/', width = 6.09, height = 5.11, units = 'in', dpi = 1600)


poly_act_summary <- report_df_filt_poly %>% 
  group_by(ACTIVITY_CODE) %>% 
  dplyr::summarise(total_acres = sum(SPATIAL_SIZE, na.rm = T)) %>% 
  mutate(type_of_treatment = if_else(str_detect(ACTIVITY_CODE, 'Road|road|Decommission|Storage|Crossing|AOP|Structure'), 'Road Related', 
                                                 if_else(str_detect(ACTIVITY_CODE, 'Riparian|riparian|Large Wood|Channel|Instream|Wetland'), 'Stream & Wetland Related', 'Other')))

poly_act_summary_groups <- poly_act_summary %>% group_by(type_of_treatment) %>% dplyr::summarise(sum_of_treatment = sum(total_acres, na.rm = T))
...
line_act_summary <- report_df_filt_line %>% 
  ...
  dplyr::summarise(total_acres = sum(SPATIAL_SIZE, na.rm = T)) %>% 
  mutate(type_of_treatment = if_else(str_detect(ACTIVITY_CODE, 'Road|road|Decommission|Storage|Crossing|AOP|Structure'), 'Road Related', 
                                                 if_else(str_detect(ACTIVITY_CODE, 'Riparian|riparian|Large Wood|Channel|Instream'), 'Stream Related', 'Other')))

line_act_summary_groups <- line_act_summary %>% group_by(type_of_treatment) %>% dplyr::summarise(sum_of_treatment = sum(total_acres, na.rm = T))
...
##### ![](inst/app/www/images/bmp_matrix.png)    
